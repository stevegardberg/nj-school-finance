...
def get_data():
    ...
    # Ensure CDS codes are strings for consistent joining
    ...
    # SAFE MERGE: Only select columns that exist in the metadata table
    meta_cols = ['cds_code', 'district_type', 'district_name']
    available_meta_cols = [c for c in meta_cols if c in df_meta.columns]
    
    if len(available_meta_cols) > 0:
        df = df.merge(df_meta[available_meta_cols], on='cds_code', how='left')
    
    # Ensure mandatory display columns exist to prevent dropdown errors
    for col in ['district_name', 'county_name', 'district_type', 'ld_display']:
