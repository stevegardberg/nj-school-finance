...
def get_data():
    ...
    df_dfg = fetch_table("dfg_mapping")

    for df in [df_sum, df_map, df_types, df_dfg]:
    ...
    if 'cds_code' in df_dfg.columns and 'dfg' in df_dfg.columns:
        df = df.merge(df_dfg[['cds_code', 'dfg']], on='cds_code', how='left')
    else:
        df['dfg'] = 'N/A'
...
sel_dfg = st.sidebar.selectbox("3️⃣ DFG:", ["All"] + sorted(df_merged['dfg'].dropna().unique().tolist()))
sel_county = st.sidebar.selectbox("4️⃣ County:", ["All"] + sorted(df_merged['county_name'].dropna().unique().tolist()))

df_f = df_merged.copy()
if sel_ld != "All": df_f = df_f[df_f['ld_display'] == sel_ld]
if sel_type != "All": df_f = df_f[df_f['district_type'] == sel_type]
if sel_dfg != "All": df_f = df_f[df_f['dfg'] == sel_dfg]
if sel_county != "All": df_f = df_f[df_f['county_name'] == sel_county]

sel_district = st.sidebar.selectbox("5️⃣ Select District:", ["Select..."] + sorted(df_f['district_name'].dropna().unique().tolist()))

# Navigation
page = st.sidebar.radio("Navigation", ["Financial Ledger", "Comparative Analysis"])
...
if page == "Financial Ledger":
    ...
    if sel_district != "Select...":
        ...
        for name, group_col in [("Legislative District", 'ld_display'), ("District Type", 'district_type'), ("DFG", 'dfg')]:
            val = target_data[group_col].iloc[0]
            st.subheader(f"🏛️ {name} Average: {val}")
            peers = df_merged[df_merged[group_col] == val].groupby('fiscal_year').mean(numeric_only=True).reset_index()
            peers = peers[peers['fiscal_year'].isin(target_years)]
