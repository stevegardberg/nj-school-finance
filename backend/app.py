...
# 4. Critical Debug: Check the district name column
st.write("DEBUG: Columns in merged df:", df_merged.columns.tolist())
district_col = 'district_name' if 'district_name' in df_merged.columns else None

if district_col:
    # Remove nulls and ensure we have unique values
    districts = sorted([str(d) for d in df_merged[district_col].dropna().unique() if d and d != 'nan'])
    st.write(f"DEBUG: Found {len(districts)} districts.")
else:
    st.error("Column 'district_name' not found in merged data!")
    st.stop()
...
if sel_district != "Select...":
    target = df_merged[df_merged[district_col] == sel_district]
