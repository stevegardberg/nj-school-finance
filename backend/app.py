...
def get_data():
    ...
    # Merge only using proven tables
    ...
    if 'district_name_y' in df.columns: df['district_name'] = df['district_name_y']
    
    return df

# 3. METRICS
def add_metrics(df):
    potential_cols = ['actual_state_aid', 'actual_tax_levy', 'adequacy_budget']
...
sel_district = st.sidebar.selectbox("3️⃣ Select District:", ["Select..."] + sorted(df_f['district_name'].dropna().unique().astype(str).tolist()))
